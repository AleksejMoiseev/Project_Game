...
from .dataclasses import Cart, Customer, Order, OrderLine, Product
...
class Checkout:
    ...
    def add_product_to_cart(
        self, sku: str, quantity: conint(ge=1) = 1, customer_id: int = None
    ):
        product = self.products_repo.get_by_sku(sku)
        if product is None:
            raise errors.NoProduct(sku=sku)
...
class Orders:
    ...
    @validate_arguments
    def get_order(
        self, number: int, customer_id: Optional[int] = None
    ) -> Order:
    ...
    def send_message_to_manager(self, order_number: int):
        ...
        if order is None:
            raise errors.NoOrder(number=order_number)

        self.mail_sender.send(
            'admin@example.com', 'New order placed!',
...
class Customers:
    ...
    @join_point
    @validate_arguments
    def get_info(self, customer_id: int):
        customer = self.customers_repo.get_by_id(customer_id)
        if not customer:
            raise errors.NoCustomer(id=customer_id)
